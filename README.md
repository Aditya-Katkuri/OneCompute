...
dozen example workloads (CPU/GPU/AI), and 498 passing tests. We are ready to begin integration
...
├── tests/              498 tests across orchestrator, worker, jobkit, isolation, trust
...
```bash
uv run pytest -q   # 498 passed, 2 skipped (500 collected; 2 Docker-only skips)
```
