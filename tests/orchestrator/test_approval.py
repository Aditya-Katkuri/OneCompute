...
def test_gated_worker_is_pending_then_approved_then_leases():
    ...
    assert client.get("/jobs/next", params={"worker_id": "cpu-1"}).status_code == 204

    # Heartbeat reports the worker is still not approved.
    hb = client.post("/heartbeat", json={"worker_id": "cpu-1"})
    ...
    assert client.post("/heartbeat", json={"worker_id": "cpu-1"}).json()["approved"] is True
    state = client.get("/state").json()
    ...
    nxt = client.get("/jobs/next", params={"worker_id": "cpu-1"})
