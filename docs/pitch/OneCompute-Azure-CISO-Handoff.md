...
- **The opportunity is measured, not modeled.** `docs/Financial_Impact.md` frames a modeled Year-1 gross of ~$125.6M Azure-equivalent capacity; the pilot replaces the modeled inputs with data. `scripts/business_case.py` turns a real device profile into a defensible Azure-equivalent dollar range using **measured** recoverable headroom, measured awake-hours (bucket coverage), and the measured AC fraction (so a laptop is only credited while plugged in). A voluntary measurement pilot is running now (`docs/measurement-pilot.md`); its readout is the number to bring to this review.
