...
|---|---|---|---|---|
| CC6.1 Logical access is restricted | Per-worker bearer token issued at registration; constant-time comparison; unknown/mismatched token rejected. **Optional device-identity binding** (`--bind-device-identity`) additionally requires the worker's TLS client-cert SHA-256 fingerprint to match the one bound at registration, so a stolen token from another host fails closed | **Implemented** | `src/orchestrator/app.py` (`compare_digest`, `_require_worker_token` fingerprint check); binding stored `workers.cert_fingerprint`; `tests/orchestrator/test_device_identity.py`; `docs/device-identity.md` | Intune/Entra-managed device identity + full SSO/OIDC is the upgrade; in production the fingerprint is injected by the mTLS terminator |
...
| CC6.1 Submission authorization | Optional operator token (`--submit-token` / `$ONECOMPUTE_SUBMIT_TOKEN`) gates job/workload submission (`Authorization: Bearer`, constant-time compare, `auth_failed` audited on failure) | **Implemented** (optional) | gate `app.py` (`_require_submit_token`, applied on `POST /jobs` and `POST /workloads`); client `src/workloads/submit.py`, `scripts/submit_jobs.py` | Full submitter SSO/OIDC is **Roadmap**; the token is the PoC step (a pilot uses controlled submitters) |
...
| TSC point | OneCompute control | Status | Citation | Gap / note |
|---|---|---|---|---|
| CC8.1 Authorized changes only | Frozen data contracts and SQLite schema treated as deliberate seams; tests mirror source layout | **Implemented** | `src/contracts/models.py`, `src/contracts/schema.sql`; `tests/` | 498 tests, 2 skipped (Docker-only) |
...
| C1.1 Data minimization off-device | Rich hour-of-week/idle profile remains local. Current workers send only a compact pseudonymous capacity/availability summary, with no idle field, hourly buckets, raw timestamps, hostname by default, or live resource stream. Legacy reports are collapsed and discarded. | **Implemented** | `src/worker/agent.py`; `src/contracts/models.py`; `src/orchestrator/app.py` (`_sanitize_profile_report`); `src/contracts/schema.sql` | Broad availability totals still require Privacy/CELA review |
| C1.2 Disposal | Job inputs/outputs are wiped at job end. Observer `-Purge` deletes the local profile, legacy telemetry/rotations, and observer ID; operator disconnect deletes the latest central summary. | **Implemented** | `runner.py`; `scripts/observe_me.ps1`; `scripts/install_observer.ps1`; `src/orchestrator/app.py` disconnect path | Pseudonymous audit retention remains time-limited by pilot policy |
...
| TSC point | OneCompute control | Status | Citation | Gap / note |
|---|---|---|---|---|
| P1 Notice / consent | Voluntary informed opt-in; dedicated measurement notice; instant withdrawal and explicit purge | **Partial** | `docs/pitch/OneCompute-Measurement-Pilot-Consent.md`; observer scripts | Formal DPIA and regional consultation remain blocking review items |
| P4 Use limitation | Rich profile stays local; compact pseudonymous capacity/availability summary only; no idle field, hourly pattern, live stream, or content collection | **Implemented** | `src/worker/agent.py`; `src/contracts/models.py`; `src/orchestrator/app.py` | Enrollment map can re-identify and must be restricted |
| P6 Access / P7 Quality | Employee controls participation and local purge; operator reads are authenticated; report values are clamped and reviewed as self-reported estimates | **Partial** | observer scripts; admin gate in `src/orchestrator/app.py` | Formal subject-access and audit-retention workflow remains a DPIA deliverable |

---

## 6. Summary: implemented vs. roadmap

**Implemented in code (the technical heart of the design):** authenticated worker calls, collision-safe certificate-bound enrollment, admission gating, separate submit/admin credentials, authenticated operator reads, signed + hash-bound + expiring manifests, optional out-of-band pinned signer, OS-enforced isolation with a fail-closed switch, lease/requeue, server-assigned append-only crediting, challenge verification, tamper-evident audit, result caps, TLS/mTLS, rate limiting, supply-chain evidence, and a privacy-minimized measurement path with no sample timeline, live utilization stream, hourly/idle upload, or hostname-derived default identity.

**Roadmap (named, not built here):** full submitter SSO/OIDC (an optional operator submit token is shipped as the PoC step), Intune/Entra-managed device certificates (optional device-identity fingerprint binding is shipped as the PoC step), HSM-custodied signing key, transparency-logged keyless cosign/OIDC/Rekor + full SLSA build levels (an offline signed attestation and the cosign integration are shipped), TLS-on-by-default with automated cert issuance/rotation, SIEM alerting thresholds + external audit anchoring (a hash-chained tamper-evident log with JSONL export is shipped), MXC validation against a real `wxc-exec` runtime (its launch-path wiring is proven via a stub harness), NPU job execution (detection/advertisement is shipped), and a formal DPIA.
