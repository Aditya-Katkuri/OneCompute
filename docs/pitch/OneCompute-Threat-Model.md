...
| Related docs | `idea.md` (concept), `architecture.md` (design), `mxc-sandbox.md` (MXC backend design + preview caveats), `soc2-alignment.md` (control-to-code mapping), `pilot-security-approval.md` (sanction runbook), `pilot-plan.md` (pilot operations), `OneCompute-Risk-Memo.docx` |
...
6. **Supply-chain compromise of the agent.** Mitigate: code signing, build provenance, dependency pinning, SBOM (section 14); roadmap cosign/Rekor transparency.
...
- **Credit integrity:** **server-assigned** credit (accepted_units x class_weight), never self-reported TOPS; append-only ledger; lease ownership prevents double-credit; capped multipliers.
...
- **Dependencies:** the sandboxed job payload is **pure-stdlib** by design (`jobkit` uses only `urllib`), so no third-party code runs inside the container - a real strength. The control plane and worker use `fastapi`/`uvicorn` (API), `pydantic` (frozen contracts), `httpx` (worker outbound client), `psutil` (headroom/idle governor), and `cryptography` (Ed25519 sign/verify - the trust root). **Two honest gaps to close before a pilot:** (1) `cryptography` (the trust root, currently 49.0.0) is pulled in *transitively* (via `azure-identity`/`msal`), not pinned as a direct dependency - it should be promoted to a pinned, CVE-watched direct dep; (2) the declared set is broad (borrowed from the CoworkBench toolbox: `anthropic`, `openai`, `pandas`, `playwright`, `matplotlib`, `pymupdf`, ...), much of it unrelated to the worker/orchestrator runtime - a hardened agent build should trim to the actual import surface. SBOM generation remains roadmap.
...
| Device-code admission gate | B1/B3 unauthorized join |
