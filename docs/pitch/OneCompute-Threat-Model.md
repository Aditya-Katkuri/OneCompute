...
**Scope of this revision:** the proof-of-concept (PoC) as built, and the proposed contained pilot. It is explicit about what is enforced in code today versus what is honest roadmap. v1.8 hardens unattended observer reliability and least privilege: atomic and validated profile persistence, a single-writer lock, fail-safe sensor handling, GPU-contributor integrity, offline enrollment retry, server-side heartbeat minimization, participant-context deployment, and complete cross-launcher purge.
...
| 1.8 | this rev | Observer lifecycle and data-integrity hardening: writeability probe; atomic, synced, size-bounded profile persistence with corrupt-file recovery; OS-backed single-writer lock; five-second minimum cadence; one-minute local saves; CPU/RAM/GPU fail-safe sampling; explicit GPU contributor counts; local collection through registration outages and pending approval; server-side measurement heartbeat minimization; participant-context limited-privilege installers; fixed interpreter paths; and purge across Scheduled Task, Startup, lock, temporary, recovery, identity, and configuration artifacts |
...
|---|---|---|---|---|---|
| Rich usage profile (hour-of-week CPU/GPU/RAM/AC/idle plus first/last successful sample times and compact availability totals) | on-device only | Personal / sensitive | **No** | rolling local state until purge | never uploaded; measurement mode writes no per-sample timeline |
| Compact measurement summary | device -> orchestrator | Low-Medium | Yes | latest row and server-side receipt time per observer; deleted on disconnect; pilot DB retention policy | pseudonymous ID, device class, coverage, aggregate capacity/AC/availability only; no hourly buckets, idle field, device-generated sample timestamps, or hostname by default |
...
| R4 | Privacy harm / employee-monitoring perception | M | H | **High** | rich profile local only; compact central schema; no hourly/idle pattern, sample timeline, or live resource stream; server-side measurement heartbeat minimization; pseudonymous ID; consent; complete cross-launcher purge; DPIA + CELA | Low-Med technically, Med pending CELA/works-council review |
...
| R16 | Falsified or corrupted measurement distorts business case | M | M | **Med** | certificate-bound source identity; pending-upload gate; server clamps; overwrite-only latest summary; atomic validated local persistence; single-writer lock; invalid readings excluded per metric; CPU-only devices excluded from GPU estimates; clock discontinuities ignored; outlier and duplicate-pattern review; sample comparison to approved endpoint-management data | Med, because values are self-reported and not hardware-attested |

---

## 20. Residual risk and acceptance
The PoC enforces authenticated, signed, sandboxed, verified, audited, opt-in, instant-yield execution controls. The measurement path now adds privacy-minimized reporting, mandatory remote HTTPS, a sanctioned mTLS/device-binding preset, authenticated operator reads, fail-safe unattended persistence, limited-privilege participant deployment, and complete purge. The honest residual concentrates in software rather than hardware-TEE isolation for any future job phase, GPU isolation, MXC real-runtime validation, self-reported measurement integrity, certificate/token operations, and unresolved legal/privacy review. **Therefore the ask is a staged 50-100 device measurement pilot, not job execution or rollout.**
...
| Atomic validated profile persistence, single-writer lock, fail-safe sensors, GPU contributor count, server clamping, certificate-bound source, and measurement outlier review | R16 |
