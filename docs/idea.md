...
- **Reporting can model wake potential before the control exists.** The measurement profile tracks long no-sample gaps, which can represent sleep, shutdown, reboot, or observer downtime. The business-case report shows the current awake-only value beside a default wake-enabled potential that assumes every gap is powered and usable at 75% CPU. That larger figure is explicitly modeled and does not change the worker's shipped no-wake policy.

The throughline: **measurement is passive and never wakes anything; the shipped worker treats every laptop as use-only, awake-only, AC-only; any future wake path requires an explicit, IT-sanctioned control plane.**
