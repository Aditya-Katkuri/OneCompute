...
**One-week voluntary measurement pilot (no job execution).** In parallel, run a short, opt-in measurement-only pilot across 50-100 managed Windows laptops, desktops, and dev boxes. It pulls and runs **no jobs**. It learns each machine's richer idle-headroom envelope locally and sends only a compact pseudonymous fleet summary, with no live utilization stream, hourly/idle pattern, or raw timestamps. Retail Xbox requires a future signed native collector or sanctioned dev-kit environment.
