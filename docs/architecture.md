...
**Usage profile (`src/worker/profiler.py`):** a rolling, **on-device** store of per-(hour-of-week) min / avg / peak CPU/GPU/RAM plus AC/idle aggregates, learned **only from job-free samples** so the envelope reflects the employee's usage and not the worker's own job. The rich profile remains local. Measurement mode uploads only a pseudonymous compact capacity/availability summary, with no hourly buckets, idle field, raw timestamps, hostname by default, sample timeline, or live resource stream.
...
| **Device identity** | optional **`--bind-device-identity`** ties the worker token to the SHA-256 fingerprint derived directly from the TLS-verified peer certificate, so a token replayed from another device fails closed (audited `device_fingerprint_mismatch`); client-supplied fingerprint headers are ignored; see [`device-identity.md`](./device-identity.md) | Intune/Entra-managed, preferably TPM-backed device certs; automated rotation/revocation |
