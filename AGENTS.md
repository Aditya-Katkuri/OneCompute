...
dashboard wired to the orchestrator API, a dozen example workloads, and 498 passing tests.

## How to verify it yourself (do this first)

```bash
uv sync --extra dev
uv run pytest -q                 # expect: 498 passed, 2 skipped (Docker-only)
...
| `tests/` | 498 tests covering matching, auth/approval, leases/requeue, crediting, yield, isolation (incl. MXC policy + runner), trust (incl. pinned-key provenance + fail-closed isolation), measurement-only profiling + fleet measurement rollup, and end-to-end flows. |
